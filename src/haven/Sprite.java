...
public abstract class Sprite {
    ...
    public static final Comparator<Part> partidcmp = new Comparator<Part>() {
        ...
        public int compare(Part a, Part b) {
            ...
            if (emergency == null) {
                System.err
                        .println("Could not impose ordering on distinct sprite parts, invoking emergency protocol!");
    ...
    public static class DynFactory implements Factory {
        ...
        public Sprite create(Owner owner, Resource res, Message sdt) {
            try {
                try {
                    Constructor<? extends Sprite> m = cl.getConstructor(
                            Owner.class, Resource.class);
                    return (m.newInstance(owner, res));
                } catch (NoSuchMethodException e) {
                }
                try {
                    Constructor<? extends Sprite> m = cl.getConstructor(
                            Owner.class, Resource.class, Message.class);
                    return (m.newInstance(owner, res, sdt));
                } catch (NoSuchMethodException e) {
                }
                throw (new ResourceException(
                        "Cannot call sprite code of dynamic resource", res));
            } catch (IllegalAccessException e) {
                throw (new ResourceException(
                        "Cannot call sprite code of dynamic resource", e, res));
            } catch (java.lang.reflect.InvocationTargetException e) {
                throw (new ResourceException(
                        "Sprite code of dynamic resource threw an exception",
                        e.getCause(), res));
    ...
    public static Sprite create(Owner owner, Resource res, Message sdt) {
        if (res.loading)
            throw (new RuntimeException(
                    "Attempted to create sprite on still loading resource"));
        ...
        if (e != null) {
            try {
                ...
                throw (new ResourceException(
                        "Error in sprite creation routine for " + res, exc, res));
            }
        }
        ...
        throw (new ResourceException("Does not know how to draw resource "
                + res.name, res));
    }
    ...
    public static void setup(Collection<? extends Part> parts, Drawer d,
                             Coord cc, Coord off) {
        for (Part p : parts) {
            p.setup(cc, off);
            d.addpart(p);
