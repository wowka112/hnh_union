...
public class Partyview extends Widget {
    ...
    Button leave = null;
    public static Partyview instace;
    ...
    private void update() {
        if (party.memb != om) {
            ...
            List<Map.Entry<Member, Avaview>> wl = new ArrayList<Map.Entry<Member, Avaview>>(
                    avs.entrySet());
            Collections.sort(wl, new Comparator<Map.Entry<Member, Avaview>>() {
                public int compare(Entry<Member, Avaview> a,
                                   Entry<Member, Avaview> b) {
    ...
    public void click(int index, int btn) {
        int c = 0;
        for (Member m : avs.keySet()) {
            if (c == index) {
                wdgmsg("click", m.gobid, btn);
                return;
            }
            c++;
        }
